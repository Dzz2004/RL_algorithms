...
def train():
    ...
    #保存模型
    torch.save(agent.actor.state_dict(), '../Policy//PPO_policy_net.pth')
    torch.save(agent.critic.state_dict(), '../Policy//PPO_value_net.pth')
    ...
    mv_return = moving_average(return_list, 9) # 9个数据的移动平均
...
def test():
    ...
    agent.actor.load_state_dict(torch.load('../Policy//PPO_policy_net.pth'))
    agent.actor.eval()
    agent.critic.load_state_dict(torch.load('../Policy//PPO_value_net.pth'))
    agent.critic.eval()
    ...
    for i_episode in range(num_episodes):
        ...
        while not done:
            ...
            episode_return += reward
        return_list.append(episode_return)
        print(f'Episode {i_episode}/{num_episodes}, Total Reward: {episode_return}')
    env.close()
